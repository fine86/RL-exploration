...
class exploreEnv(object):
    ...
    def observation(self):
        ...
        ### 수정 완. 1000->1500
            ...
            self.done = True
        #3트
        ### success에 의한 reward를 훨씬 줄여줌
        ### 수정 완. 60->30
        elif success:
            self.positive_reward += 30
        ...
        #3트
        ### 수정 완. 0,3 -> 0.6
        ###### 탐색 많이 할수록 reward 크게 주는 방향으로 설정
        else:
            self.positive_reward += observed_areas * 1.0 * (math.sqrt(current_distance / (self.size * math.sqrt(2))) + 0.3)
        #3트
        ### clip 추가
        ### step이 짧을수록 reward 크게 받게 설정했는데 이러면 local minima에 빠질것 같아서 폐기
        if self.done or self.steps == self.max_steps:
            ...
            #self.positive_reward += 3000 * (self.max_steps - self.steps) / self.max_steps
...
while total_steps < max_step:
    ...
    for i in range(horizon):
        ...
        if done:
            observed = (exploration.whole_area - exploration.observable_area) / exploration.whole_area
            wandb.log({"Exploration Boundary " : observed})
